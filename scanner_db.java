...
class scanner_db extends db_helper {
	...
	scanner_db() {
		...
		try{
			statement.execute( "create table if not exists " + MODULES_TABLE + " (id integer, name text, price decimal, date_added text, date_modified text, sales integer, date text )" );

			// statement.execute( "create table if not exists " + STAT_TABLE + " (id integer, price decimal, date text, sales integer )" );
	...
	int getLastId() {
		...
		try {
			ResultSet r = statement.executeQuery( "SELECT MAX(id) as max from " + MODULES_TABLE );
			id = r.getInt( "max" );
	...
	ResultSet getWorthModules() {
		ResultSet ret = null;

		try {
			// More than 5000 sales/downloads or more than 500$ per month
			ret = statement.executeQuery( "SELECT id FROM " + MODULES_TABLE + " WHERE sales > 5000 OR ( ( sales * price ) / (julianday( 'now' ) - julianday( date_added ) ) ) > 5" );
	...
	void saveData( HashMap<String, String> pageData ) {
